...
- [ ] 2.1 Full unit suite passes in UTC
- [ ] 2.2 Full unit suite passes in a non-UTC process zone
- [ ] 2.3 Type checking passes
- [ ] 2.4 Linting passes
- [ ] 2.5 Cloudflare production build succeeds

#### Manual

- [ ] 2.6 With no server date seed, Today shows the explanatory non-actionable state and never exposes future plants as due
- [ ] 2.7 In UTC− and UTC+ browser zones where the local day differs from UTC, Today, Add, Edit, `/plants`, and plant detail agree after hydration
- [ ] 2.8 Add, Update, Watered, and Postpone persist the fresh browser-local date and select the matching seasonal interval
- [ ] 2.9 Browser date acquisition failure blocks every dated mutation with a retryable explanation and sends no Action request
- [ ] 2.10 Add and Edit previews refresh after midnight, while an immediate cross-midnight submit follows the accepted fresh-date-without-reconfirmation contract
- [ ] 2.11 Midnight, `pageshow`, and visibility recovery update all relative surfaces without a document reload
- [ ] 2.12 Overdue labels, non-color urgency cues, focus recovery, undo, and reduced-motion behavior remain intact
- [ ] 2.13 A device clock set more than one calendar day from UTC is rejected by every dated mutation with clock-specific copy and no dedicated Retry action in the error notice, never the generic connection message; normal action controls remain available after correction
