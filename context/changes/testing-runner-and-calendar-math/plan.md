...
- [ ] 3.3 The first push (PR or direct to `main`) shows CI checks running
- [ ] 3.4 `setup-node` resolves the pnpm cache without a lockfile error
- [ ] 3.5 Deliberate break turns the job red at `pnpm test`; revert to green
- [ ] 3.6 `lint` and `build` pass in CI for the first time
