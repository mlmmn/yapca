...
2. **What are the calendar boundaries between growing and dormancy season, and are they fixed or user-configurable?** — FR-015 relies on automatic date-based season switching, but the exact boundary dates (e.g. growing Mar–Oct, dormancy Nov–Feb) and whether the user can adjust them are undecided. Owner: user. Blocks correct interval selection once implemented.
